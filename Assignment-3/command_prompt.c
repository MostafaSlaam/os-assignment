...
//Q1:Find EXACT Physical Address (1.5 MARK)
//=========================================
/*DON'T change this function*/
...
int FindPhysicalAddress(char** arguments)
{
	...
	uint32 *ptr=NULL;
	get_page_table(ptr_page_directory, va1, 0, &ptr) ;
	if(ptr!=NULL)
	{
		uint32 TableEntry=ptr[PTX(va1)];
		int p = ptr[PTX(va1)] & PERM_PRESENT;
        if(p!=0)
         {
		int frameNum=TableEntry>>12;
		uint32 address=frameNum*PAGE_SIZE;
		uint32 offset=PGOFF(va1);
...
//Q2:Share Range with Permissions (1 MARK)
//========================================
...
void ShareRangeWithPermissions(char** arguments)
{
	...
	char *va2 = (char *)strtol(arguments[2], NULL, 16) ;
			char p=arguments[4][0];
			uint32 size = strtol(arguments[3], NULL, 10) ;
			size *= 1048576 ; //convert it to bytes
			...
			for (i = 0 ; i < size ; i += PAGE_SIZE)
			{
				...
				if (ptr_page_table1 != NULL)
				{
                   uint32 *ptr_page_table2 = NULL;
					...
					if(p=='r')
						{
						ptr_page_table2[PTX(va2)]&=~(PERM_WRITEABLE);
					    }
...
//Q3: Find virtual address of the given frame number (1.5 MARK)
//=============================================================
...
{
	    uint32 *ptr=NULL;
		get_page_table(ptr_page_directory, add, 0, &ptr) ;
		if(ptr!=NULL)
		{
			int p = ptr[PTX(add)] & PERM_PRESENT;
			        if(p!=0)
			        {
			uint32 TableEntry=ptr[PTX(add)];
			int frameNum=TableEntry>>12;
			if(frameNum==x)
				return (int)add;
			        }
...
//BONUS: Connect page number to frame number with permissions (1.5 MARK)
//======================================================================
...
uint32 ConnectPageToFrame(char** arguments)
{
	...
	//put your logic here
	//...
  int pageNum=strtol(arguments[1],NULL,10);
  uint32 va1=pageNum*4096;
  int framNum=strtol(arguments[2],NULL,10);
  char x=arguments[3][0];
  char y=arguments[4][0];
  uint32 *ptr=NULL;
  get_page_table(ptr_page_directory,(void*) va1, 0, &ptr) ;
  ptr[PTX(va1)]=framNum;
  ptr[PTX(va1)]= ptr[PTX(va1)]<<12;
  ptr[PTX(va1)]= ptr[PTX(va1)]|PERM_PRESENT;
  ptr[PTX(va1)]= ptr[PTX(va1)]|PERM_AVAILABLE;
  ptr[PTX(va1)]= ptr[PTX(va1)]&(~PERM_MODIFIED);
  ptr[PTX(va1)]= ptr[PTX(va1)]&(~PERM_USED);
  if(x=='w')
  {
	  ptr[PTX(va1)]= ptr[PTX(va1)]|PERM_WRITEABLE;
  }
  else if(x=='r')
  {
	  ptr[PTX(va1)]= ptr[PTX(va1)]&(~PERM_WRITEABLE);
  ...
  if(y=='u')
   {
 	  ptr[PTX(va1)]= ptr[PTX(va1)]|PERM_USER;
   }
   else if(y=='s')
   {
 	  ptr[PTX(va1)]= ptr[PTX(va1)]&(~PERM_USER);
   }
	return(uint32) ptr[PTX(va1)];
}
